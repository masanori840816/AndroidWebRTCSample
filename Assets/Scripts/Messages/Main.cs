    ...
    private void HandleMessage(string receivedData)
    {
        ...
        switch(message.@event)
        {
            case "offer":
                ...
                break;
            case "candidate":
                RTCIceCandidate candidateMessage = JsonUtility.FromJson<RTCIceCandidate>(message.data);
                if(candidateMessage == null)
                {
                    Debug.Log($"Invalid candidate:{message.data}");
                    return;
                }
                _webrtcCtrl.OnCandidateReceived(candidateMessage);
                break;
            default:
